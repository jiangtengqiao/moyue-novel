...
async function seedData() {
  ...
  for (const [name, display, order] of cats) {
    await pool.query('INSERT INTO categories (id, name, display_name, sort_order) VALUES ($1, $2, $3, $4)', 
      [require('crypto').randomUUID(), name, display, order]);
  }
  ...
  await pool.query(`INSERT INTO novels (id, title, author, creator_id, category_id, description, tags, status, word_count, chapter_count, featured) 
    ...
    [novelId, '星河彼岸', '官方编辑部', creatorId, catId, '在浩瀚星河的尽头，少年踏上一段寻找自我与真相的旅程。', JSON.stringify(['玄幻','冒险','成长']), 'ongoing', 0, 0, true]);
  ...
  await pool.query('UPDATE novels SET word_count = $1, chapter_count = $2 WHERE id = $3',
    [chapters.reduce((s, c) => s + c[1].replace(/\s/g, '').length, 0), chapters.length, novelId]);

  console.log('[墨阅小说] 种子数据已创建 (admin / admin123)');
}
